...
class Cpu < Cpucell
    ...
    def initialize(board, opp_board)
        ...
        num = []
        @coordinates = []
        @ships = [destroyer = Ship.new(2, "(D)"), submarine = Ship.new(3, "(S)"), cruiser = Ship.new(4, "(C)"), battleship = Ship.new(5, "(B)")]
        counter = 0
        @board.size.times do 
            num << counter
            counter += 1
    ...
    def deploy_opp_ships()
        ...
        4.times do 
            while true
                ...
                if @board.main(@ships[counter], hold_it[0], hold_it[1], pos.sample) != "Invalid Placement!"
                    counter += 1
